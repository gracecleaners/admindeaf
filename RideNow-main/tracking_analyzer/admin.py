...
class TrackerAdmin(admin.ModelAdmin):
    ...
    def changelist_view(self, request, extra_context=None):
        ...
        if request.method == 'GET':
            ...
            paginator = response.context_data['cl'].paginator

            # Check if the current page + 1 is a valid page.
            if current_page + 1 <= paginator.num_pages:
                current_pks = paginator.page(current_page + 1).object_list.values_list('pk', flat=True)
                
                # Get the requests count for the objects in the current page.
